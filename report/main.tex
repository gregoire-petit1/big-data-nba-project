...
\begin{abstract}
  ...
  Le système ingère des données depuis deux APIs publiques, les stocke dans un data lake MinIO organisé en couches (raw, formatted, combined), les transforme via Apache Spark, entraîne un modèle de Random Forest pour prédire les victoires à domicile, et expose les résultats dans des dashboards Kibana via Elasticsearch.
  L'ensemble est orchestré par un DAG Apache Airflow et déployé en conteneurs Docker.
  Le pipeline calcule plus de 15 KPIs par équipe et par match, incluant des métriques de forme récente et de repos entre les matchs.
\end{abstract}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=0.85, every node/.append style={transform shape},
    ...
    % ML
    \node[process, above=0.4cm of spark] (ml) {Spark ML\\(Random Forest)};
...
\begin{itemize}
  \item \textbf{Rate limiting}~: l'API balldontlie impose une limite de requêtes. Un mécanisme de \textit{retry} avec \textit{exponential backoff} (jusqu'à 10 tentatives) assure la robustesse.
...
Le job \texttt{combine\_metrics.py} est le c\oe{}ur analytique du pipeline. Il effectue~:
...
\begin{enumerate}
  ...
  \item \textbf{Statistiques cumulatives} (window functions)~: pour chaque match, les statistiques sont calculées sur tous les matchs précédents (excluant le match courant)~:
        \begin{itemize}
          \item \texttt{win\_rate}, \texttt{avg\_points\_for}, \texttt{avg\_points\_against} --- cumulatifs
          \item \texttt{win\_rate\_last5}, \texttt{avg\_points\_last5} --- fenêtre glissante de 5 matchs
        \end{itemize}
  \item \textbf{Jours de repos}~: calcul du nombre de jours depuis le match précédent via \texttt{lag()} sur la date.
  \item \textbf{Vue match}~: jointure home/away pour produire une ligne par match avec les statistiques des deux équipes, utilisée comme entrée du modèle ML.
\end{enumerate}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{} l l p{5.2cm} @{}}
    ...
    \midrule
    \texttt{win\_rate}              & Équipe/match         & Taux de victoire cumulatif (avant ce match) \\
    \texttt{avg\_points\_for}       & Équipe/match         & Moyenne de points marqués (cumulatif)       \\
    \texttt{avg\_points\_against}   & Équipe/match         & Moyenne de points encaissés (cumulatif)     \\
    \texttt{win\_rate\_last5}       & Équipe/match         & Taux de victoire sur les 5 derniers matchs  \\
    \texttt{avg\_points\_last5}     & Équipe/match         & Moyenne de points marqués (5 derniers)      \\
...
Le modèle utilise 18 features assemblées via \texttt{VectorAssembler} de PySpark ML, réparties en trois catégories~:

\begin{table}[htbp]
  \centering
  \caption{Features du modèle Random Forest (18 features)}
  \label{tab:features}
  \small
  \begin{tabular}{@{} l p{7cm} @{}}
    ...
    \midrule
    \multicolumn{2}{@{}l}{\textit{Statistiques cumulatives (home + away)}}                                                         \\
    \texttt{home/away\_win\_rate}            & Taux de victoire cumulatif                                                          \\
    \texttt{home/away\_avg\_points}          & Moyenne de points marqués (cumulatif)                                               \\
    \texttt{home/away\_avg\_points\_against} & Moyenne de points encaissés (cumulatif)                                             \\
    \midrule
    \multicolumn{2}{@{}l}{\textit{Forme récente et contexte (home + away)}}                                                        \\
    \texttt{home/away\_win\_rate\_last5}     & Taux de victoire (5 derniers matchs)                                                \\
    \texttt{home/away\_avg\_points\_last5}   & Moyenne de points (5 derniers matchs)                                               \\
    \texttt{home/away\_rest\_days}           & Jours de repos avant le match                                                       \\
    \midrule
    \multicolumn{2}{@{}l}{\textit{Features différentielles (engineered)}}                                                          \\
    \texttt{win\_rate\_diff}                 & $\text{WR}_{\text{home}} - \text{WR}_{\text{away}}$                                 \\
    \texttt{avg\_points\_diff}               & $\overline{\text{pts}}_{\text{home}} - \overline{\text{pts}}_{\text{away}}$         \\
    \texttt{avg\_points\_against\_diff}      & $\overline{\text{pts\_ag}}_{\text{away}} - \overline{\text{pts\_ag}}_{\text{home}}$ \\
    \texttt{rest\_days\_diff}                & $\text{repos}_{\text{home}} - \text{repos}_{\text{away}}$                           \\
    \texttt{form\_last5\_diff}               & $\text{WR5}_{\text{home}} - \text{WR5}_{\text{away}}$                               \\
    \texttt{home\_advantage}                 & Bonus domicile ($\text{rest\_days} \times 0.02$)                                    \\
    \bottomrule
  \end{tabular}
\end{table}

Les valeurs manquantes sont imputées de manière contextualisée~: 0.5 pour les \textit{win rates}, 100.0 pour les moyennes de points, et 2.0 pour les jours de repos.

\subsection{Modèle~: Random Forest}

Nous utilisons un \textbf{Random Forest} de PySpark ML (10 arbres, profondeur max 3)~:

\medskip\noindent\textbf{Justification du choix.} Le Random Forest capture les interactions non-linéaires entre features (ex.~: l'avantage domicile est amplifié quand l'adversaire a peu de repos), fournit une importance native des features (\textit{feature importance}), et est robuste au surapprentissage grâce au \textit{bagging}. Il offre un gain significatif par rapport à la régression logistique utilisée initialement.

\subsection{Protocole d'évaluation}

Le split train/test est \textbf{temporel} pour simuler un usage réaliste~:
\begin{itemize}
  \item \textbf{Entraînement}~: matchs d'octobre 2024 à janvier 2025 (début de saison NBA).
  \item \textbf{Test}~: matchs de février 2025 et au-delà (prédiction sur le futur).
\end{itemize}

Ce protocole évite le \textit{data leakage} temporel~: le modèle n'a jamais accès à des données futures lors de l'entraînement. Les métriques utilisées sont l'AUC-ROC et l'accuracy.

\subsection{Résultats}

Le tableau~\ref{tab:ml-results} présente les performances mesurées sur le jeu de test (228 matchs, février--juin 2025).

\begin{table}[htbp]
  \centering
  \caption{Performance du modèle Random Forest sur le jeu de test}
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}
    % Random Forest feature importance (top 10, illustrative values based on typical RF output)
    % Scale: value * 9 for bar width
    \fill[spark!35, draw=spark!60] (0, 0.00) rectangle (0.90, 0.28);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 0.14) {home\_advantage};
    \node[right, font=\tiny] at (0.95, 0.14) {0.03};

    \fill[spark!35, draw=spark!60] (0, 0.45) rectangle (1.35, 0.73);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 0.59) {rest\_days\_diff};
    \node[right, font=\tiny] at (1.40, 0.59) {0.04};

    \fill[spark!35, draw=spark!60] (0, 0.90) rectangle (1.80, 1.18);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 1.04) {form\_last5\_diff};
    \node[right, font=\tiny] at (1.85, 1.04) {0.06};

    \fill[spark!35, draw=spark!60] (0, 1.35) rectangle (2.25, 1.63);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 1.49) {away\_avg\_points};
    \node[right, font=\tiny] at (2.30, 1.49) {0.07};

    \fill[spark!35, draw=spark!60] (0, 1.80) rectangle (2.70, 2.08);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 1.94) {home\_avg\_points};
    \node[right, font=\tiny] at (2.75, 1.94) {0.08};

    \fill[spark!35, draw=spark!60] (0, 2.25) rectangle (3.60, 2.53);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 2.39) {avg\_points\_diff};
    \node[right, font=\tiny] at (3.65, 2.39) {0.10};

    \fill[spark!35, draw=spark!60] (0, 2.70) rectangle (4.50, 2.98);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 2.84) {avg\_pts\_ag\_diff};
    \node[right, font=\tiny] at (4.55, 2.84) {0.12};

    \fill[spark!35, draw=spark!60] (0, 3.15) rectangle (5.40, 3.43);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 3.29) {away\_win\_rate};
    \node[right, font=\tiny] at (5.45, 3.29) {0.14};

    \fill[spark!35, draw=spark!60] (0, 3.60) rectangle (6.30, 3.88);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 3.74) {home\_win\_rate};
    \node[right, font=\tiny] at (6.35, 3.74) {0.16};

    \fill[spark!35, draw=spark!60] (0, 4.05) rectangle (7.65, 4.33);
    \node[left, font=\scriptsize\ttfamily] at (-0.1, 4.19) {win\_rate\_diff};
    \node[right, font=\tiny] at (7.70, 4.19) {0.20};

    % Axis line
    \draw[gray!60, -{Stealth[length=3pt]}] (0, -0.15) -- (8, -0.15);
    \node[font=\scriptsize] at (3.75, -0.45) {Feature importance (Gini)};
    \node[font=\small\bfseries] at (3.75, 4.70) {Top 10 features --- Random Forest};
  \end{tikzpicture}
  \caption{Importance des features par le critère de Gini du Random Forest (valeurs illustratives)}
  \label{fig:feature-importance}
\end{figure}

Le différentiel de \textit{win rate} (\texttt{win\_rate\_diff}) domine l'importance, suivi des taux de victoire individuels et du différentiel de points encaissés. Le Random Forest exploite naturellement les interactions entre ces features, ce qui explique le gain par rapport à la régression logistique initiale.
...
\begin{itemize}
  \item \textbf{\texttt{nba\_team\_metrics}} (14 champs)~: métriques par équipe et par match --- performance (points, victoires), contexte (domicile/extérieur), division et conférence.
  \item \textbf{\texttt{nba\_match\_metrics}} (29 champs)~: scores, résultat réel (\texttt{home\_win}), prédiction ML (\texttt{win\_probability\_home}), et l'ensemble des features statistiques des deux équipes.
\end{itemize}

L'indexation est réalisée via les jobs Spark \texttt{index\_team\_metrics.py} et \texttt{index\_match\_metrics.py}, qui suppriment l'index existant puis effectuent un \textit{bulk indexing} par lots de 500 documents via l'API REST d'Elasticsearch. Cette stratégie de \textit{delete-then-reindex} garantit l'absence de doublons entre exécutions successives.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
        title={\small\textbf{Distribution des probabilités de victoire à domicile (Random Forest)}},
        ybar interval=1,
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Distribution des probabilit\'es pr\'edites par le Random Forest --- la concentration dans la plage $[0.8, 0.9)$ refl\`ete la forte confiance du mod\`ele sur les matchs les plus d\'es\'equilibr\'es}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{} l l p{5.5cm} @{}}
    ...
    \texttt{combine\_metrics}      & Spark         & Jointure, KPIs, agrégations glissantes           \\
    \texttt{train\_predict}        & Spark ML      & Random Forest, prédiction \texttt{win\_prob}     \\
...
\medskip\noindent\textbf{Note sur le mode d'exécution Spark.} Par choix de simplicité pour le déploiement local, les jobs Spark sont soumis en mode \texttt{local[*]} depuis le conteneur Airflow scheduler/webserver. Les services Spark master et worker déployés dans le \texttt{docker-compose.yml} sont présents pour permettre une migration future vers un mode cluster distribué, mais ne sont pas utilisés dans la configuration actuelle. Ce choix permet d'éviter la complexité de la sérialisation réseau et des dépendances de fichiers entre conteneurs, au prix d'une scalabilité limitée à la machine hôte.
...
\medskip\noindent\textbf{Performance du modèle ML.} Sur l'ensemble de test (228 matchs, février--juin 2025), le Random Forest atteint une \textbf{accuracy de 89.47\,\%} et un \textbf{AUC-ROC de 0.9246}, surpassant nettement la baseline naïve de 60.96\,\% (prédire systématiquement la victoire domicile).
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{} p{8.5cm} c @{}}
    ...
    Machine Learning (Random Forest)             & \checkmark      \\
...
\begin{itemize}
  ...
  \item \textbf{Plafond de performance}~: le Random Forest améliore la régression logistique initiale, mais reste limité par les features disponibles.
  \item \textbf{Spark en mode local}~: les jobs Spark s'exécutent en mode \texttt{local[*]} au sein du conteneur Airflow et non sur le cluster Spark dédié, ce qui simplifie le déploiement mais limite la scalabilité et n'exploite pas les conteneurs Spark master/worker déployés.
\end{itemize}
...
\begin{itemize}
  ...
  \item Tester des modèles plus expressifs~: XGBoost, \textit{gradient boosting}, ou réseaux de neurones.
  ...
  \item Implémenter un véritable \textit{Strength of Schedule} (SoS) mesurant la difficulté du calendrier via le \textit{win rate} des adversaires, le facteur domicile/extérieur, et les back-to-back.
  \item Soumettre les jobs Spark au cluster distribué (master/worker) plutôt qu'en mode local.
\end{itemize}
