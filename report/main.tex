...
\begin{enumerate}
  ...
  \item \textbf{Strength of Schedule} (SoS)~: fenêtre \textit{look-ahead} de 5 matchs pour calculer la difficulté du calendrier.
...
\texttt{schedule\_difficulty\_next5} & Équipe/match & Difficulté du calendrier (5 prochains) \\
...
\end{axis}
\end{tikzpicture}
\caption{Distribution des probabilités prédites par le Random Forest --- la distribution bimodale autour de 0.4 et 0.6 reflète les probabilités discrètes produites par les feuilles des arbres}
...
\begin{itemize}
  ...
  \item \textbf{SoS simplifié}~: la difficulté du calendrier utilise actuellement un proxy plutôt qu'un calcul complet intégrant le \textit{win rate} des adversaires.
\end{itemize}
